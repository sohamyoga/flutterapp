...
class WidgetWorkout extends StatefulWidget {
  ModelDummySend _modelDummySend;
  List<ModelWorkoutExerciseList> _modelExerciseList;

  // int tableId;

  WidgetWorkout(this._modelExerciseList, this._modelDummySend);
...
class WidgetDetailData extends StatefulWidget {
  ...
  WidgetDetailData(
      ...
      this.muteOverCallback,this._functionSkipTick,this.backClick);
...
class _WidgetDetailData extends State<WidgetDetailData>
  ...
  void initState() {
    ...
    //     initialVideoId: YoutubePlayer.convertUrlToId(widget._modelExerciseDetail.video),
                    ...
                    children: <Widget>[
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          Expanded(
                            child: getSmallNormalTextWithMaxLine(
                                S.of(context).sound, Colors.black, 1),
                            flex: 1,
                          ),
                          Switch(
  ...
  Widget build(BuildContext context) {
    ...
    // if (widget.fromFirst) {
    //   widget.fromFirst = false;
    //   print("fromfirst2===${widget._modelExerciseList}");
    //
    //   currentTime = widget._modelExerciseList.duration;
    //   totalTimerTime = int.parse(widget._modelExerciseList.duration);
    //
...
class _WidgetWorkout extends State<WidgetWorkout> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    // initTts();
    // audioCache = AudioCache(prefix: "assets/",fixedPlayer: player);
    startTime = Constants.addDateFormat.format(new DateTime.now());
    getMutes();

    // Timer.periodic(Duration(milliseconds: 1000), (_) {
    // Timer.periodic(Duration(seconds: 1000), (_) {
    //   totalDuration++;
    //
    //   print("sectime==$totalDuration");
    ...
    if (Platform.isIOS) {
      ...
      // audioCacheBG.fixedPlayer?.startHeadlessService();
  ...
  getDetailWidget(){
    ...
    ModelExerciseDetail _modelExerciseDetail = DataFile.getExerciseDetailList()[0];
          ...
          if (isSkip) {
            ...
            if (!isReady) {
              _speak(
                  "Ready to go start with ${_modelExerciseDetail.name}");
            ...
            return WidgetDetailData(
                ...
                readyDuration,
                setReady,
                getMutesNoRefresh,playSoundTicks,backDialog);
          }
          ...
          child: Column(
            children: [
              ...
              getMediumBoldItalicText(
                  widget._modelDummySend.name, Colors.black),
              SizedBox(
                height: 7,
              ),
              SvgPicture.asset(
                      ...
                      child: Column(
                        children: [
                          getMediumBoldTextWithMaxLine(
                            ...
                            //         ._modelExerciseList.length * Constants.calDefaultCalculation)),
                      ...
                      child: Column(
                        children: [
                          getMediumBoldTextWithMaxLine(
                              ...
                              Colors.black,
                              1),
                          // getMediumBoldTextWithMaxLine(getTime.toString(), Colors.black, 1),
                          Padding(
                            padding: EdgeInsets.all(0),
                            child: getMediumTextWithWeight(
